# Mitras Users Groups Clients and Channels E2E Testing Tool
...
```bash
./e2e --help
Tool for testing end-to-end flow of Mitras by doing a couple of operations namely:
...
Use the following commands from the root Mitras directory:
...
Flags:
  ...
  -h, --help                   help for e2e
  -H, --host string            address for a running Mitras instance (default "localhost")
...
To use `-H` option, you can specify the address for the Mitras instance as an argument when running the program. For example, if the Mitras instance is running on another computer with the IP address 192.168.0.1, you could use the following command:

```bash
go run tools/e2e/cmd/main.go --host 142.93.118.47
```

This will tell the program to connect to the Mitras instance running on the specified IP address.
