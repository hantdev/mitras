# Mitras Clients and Channels Provisioning Tool
...
that can be used by Mitras MQTT benchmarking tool (`mqtt-bench`).
...
```bash
./provision --help
Tool for provisioning series of Mitras channels and clients and connecting them together.
...
Flags:
  ...
  -h, --help              help for provision
      --host string       address for mitras instance (default "https://localhost")
      --num int           number of channels and clients to create and connect (default 10)
  -p, --password string   mitras users password
      --ssl               create certificates for mTLS access
  -u, --username string   mitras user
...
```bash
go run tools/provision/cmd/main.go -u test@mitras.com -p test1234 --host https://142.93.118.47
```

If you want to create a list of channels with certificates:

```bash
go run tools/provision/cmd/main.go  --host http://localhost --num 10 -u test@mitras.com -p test1234 --ssl true --ca docker/ssl/certs/ca.crt --cakey docker/ssl/certs/ca.key
