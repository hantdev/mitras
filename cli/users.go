...
var cmdUsers = []cobra.Command{
	{
		...
		Long: "Create user with provided firstname, lastname, email, username and password. Token is optional\n" +
			"For example:\n" +
			"\tmitras-cli users create jane doe janedoe@example.com jane_doe 12345678 $USER_AUTH_TOKEN\n",
	...
	},
	{
		...
		Long: "Get all users or get user by id. Users can be filtered by name or metadata or status\n" +
			"Usage:\n" +
			"\tmitras-cli users get all <user_auth_token> - lists all users\n" +
			"\tmitras-cli users get all <user_auth_token> --offset <offset> --limit <limit> - lists all users with provided offset and limit\n" +
			"\tmitras-cli users get <user_id> <user_auth_token> - shows user with provided <user_id>\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Generate a new token with username and password\n" +
			"For example:\n" +
			"\tmitras-cli users token jane.doe 12345678\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsageCmd(*cmd, cmd.Use)
				return
...

	{
		...
		Long: "Generate new token from refresh token\n" +
			"For example:\n" +
			"\tmitras-cli users refreshtoken <refresh_token>\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 1 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Updates either user name and metadata or user tags or user email\n" +
			"Usage:\n" +
			"\tmitras-cli users update <user_id> '{\"first_name\":\"new first_name\", \"metadata\":{\"key\": \"value\"}}' $USERTOKEN - updates user first and lastname and metadata\n" +
			"\tmitras-cli users update tags <user_id> '[\"tag1\", \"tag2\"]' $USERTOKEN - updates user tags\n" +
			"\tmitras-cli users update username <user_id> newusername $USERTOKEN - updates user name\n" +
			"\tmitras-cli users update email <user_id> newemail@example.com $USERTOKEN - updates user email\n",
	...
	},
	{
		...
		Long: "Get user profile\n" +
			"Usage:\n" +
			"\tmitras-cli users profile $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 1 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Send reset password request\n" +
			"Usage:\n" +
			"\tmitras-cli users resetpasswordrequest example@mail.com\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 1 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Reset password\n" +
			"Usage:\n" +
			"\tmitras-cli users resetpassword 12345678 12345678 $REQUESTTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 3 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Update password\n" +
			"Usage:\n" +
			"\tmitras-cli users password old_password new_password $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 3 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Change user status to enabled\n" +
			"Usage:\n" +
			"\tmitras-cli users enable <user_id> <user_auth_token>\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Change user status to disabled\n" +
			"Usage:\n" +
			"\tmitras-cli users disable <user_id> <user_auth_token>\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Delete user by id\n" +
			"Usage:\n" +
			"\tmitras-cli users delete <user_id> $USERTOKEN - delete user with <user_id>\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "Search users by query\n" +
			"Usage:\n" +
			"\tmitras-cli users search <query> <user_auth_token>\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 2 {
				logUsageCmd(*cmd, cmd.Use)
				return
