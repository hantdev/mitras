...
var cmdGroups = []cobra.Command{
	...
	},
	{
		...
		Long: "Get all users groups, group children or group by id.\n" +
			"Usage:\n" +
			"\tmitras-cli groups get all $DOMAINID $USERTOKEN - lists all groups\n" +
			"\tmitras-cli groups get children <group_id> $DOMAINID $USERTOKEN - lists all children groups of <group_id>\n" +
			"\tmitras-cli groups get parents <group_id> $DOMAINID $USERTOKEN - lists all parent groups of <group_id>\n" +
			"\tmitras-cli groups get <group_id> $DOMAINID $USERTOKEN - shows group with provided group ID\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) < 3 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "List users in a group\n" +
			"Usage:\n" +
			"\tmitras-cli groups users <group_id> $DOMAINID $USERTOKEN",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 3 {
				logUsageCmd(*cmd, cmd.Use)
				return
	...
	},
	{
		...
		Long: "List channels in a group\n" +
			"Usage:\n" +
			"\tmitras-cli groups channels <group_id> $DOMAINID $USERTOKEN",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 3 {
				logUsageCmd(*cmd, cmd.Use)
				return
...
var groupAssignCmds = []cobra.Command{
	{
		...
		Long: "Assign users to a group\n" +
			"Usage:\n" +
			"\tmitras-cli groups assign users <relation> '[\"<user_id_1>\", \"<user_id_2>\"]' <group_id> $DOMAINID $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 5 {
				logUsageCmd(*cmd, cmd.Use)
				return
...
var groupUnassignCmds = []cobra.Command{
	{
		...
		Long: "Unassign users from a group\n" +
			"Usage:\n" +
			"\tmitras-cli groups unassign users <relation> '[\"<user_id_1>\", \"<user_id_2>\"]' <group_id> $DOMAINID $USERTOKEN\n",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) != 5 {
				logUsageCmd(*cmd, cmd.Use)
				return
