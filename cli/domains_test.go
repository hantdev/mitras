...
func TestGetDomainsCmd(t *testing.T) {
	...
	var dom mitrassdk.Domain
	var page mitrassdk.DomainsPage

	cases := []struct {
		desc          string
		args          []string
		sdkErr        errors.SDKError
		page          mitrassdk.DomainsPage
		domain        mitrassdk.Domain
		logType       outputLog
		errLogMessage string
	}{
		{
			...
			page: mitrassdk.DomainsPage{
				Domains: []mitrassdk.Domain{domain},
			},
			logType: entityLog,
		},
		{
...
func TestListDomainRoleMembersCmd(t *testing.T) {
	...
	page := mitrassdk.RoleMembersPage{
		Total:   1,
		Offset:  0,
		Limit:   10,
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			switch tc.logType {
			case entityLog:
				var page mitrassdk.RoleMembersPage
				err := json.Unmarshal([]byte(out), &page)
				assert.Nil(t, err)
